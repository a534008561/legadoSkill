...
> **Next.js RSC flight chaotic_payload 解码（2026-09-09）**：生肉漫画站 NicoManga 破解沉淀的**通用类站点方法论** — 识别 Next.js/Turbopack 壳站数据混淆（密文 = UTF8字节⊕循环密钥+19968，严格落 U+4E00-U+4EFF）→ chunk 反查算法 → Python 离线验证 → Rhino 兼容 jsLib 移植（push 反转义拼接/最长CJK段定位/手写UTF8解码/大括号配平截JSON，零 TextDecoder 零 apply 大数组）→ chapterList/bookList 走 NativeObject 数组键值直取绕 AnalyzeByJSoup 污染 → 图片源 bookSourceType=2 + UA 防盗链三连测。含「站点上限 vs 规则缺陷」判定五步法、新旧混合站 SSR 入口优先策略、**MCP save_source 零转义写法**（全书源 JS 零反斜杠零内嵌双引号，根治双重转义损坏）。方法主干 [references/方法-NextJS-RSC-flight的chaotic_payload解码.md](references/方法-NextJS-RSC-flight的chaotic_payload解码.md)，> **APP接口聚合书源拆解（2026-09-09）**：s.wendulou.com（小小阅读/书香之家app，模板by遇知，解密by尐哖&landseer）多App共用后端JSON API聚合站方法论 — ①**子域推导**：`source.getKey().replace('//s','//子域')`，全链路URL从getKey()派生→换主域即换线路零改规则（实测mysignal.cn/chuangke.tv同库存活，pjxhmy等5域名DNS已死）②**鉴权破坏性实验法**：sign/time实测不校验（乱填照常返回），真鉴权=UA尾部`_{package}`后缀——逐项单变量破坏定位，别给装饰参数写逻辑③**双层加密体系**：动态种子信封AES-256-CBC（base64密文=16字节key种子+密文体+16字节IV种子；key=SHA256(种子hex)；**IV=md5(IV种子)的hex字符串前16个ASCII字节⊕种子⊕0xFF——java.md5Encode返回hex字符串不是raw digest**！CBC错IV只毁首块错key全乱=定位铁律）+固定密钥AES-128（`java.aesBase64DecodeToString(result,key,trans,iv)`四参解目录章名/正文）④**单书源多站换源**：init读`book.getVariable("custom")`选来源→tocUrl动态指向`catalog/{来源站ID}/path`→preUpdateJs=`java.refreshTocUrl()`刷新生效，来源选用率写remark展示⑤bookSourceComment当JS公共库（eval(String(source.bookSourceComment))加载decode）⑥发现页动态分类树（classify/v4/index接口不硬编码+{url:null}分组标题行+Flex style网格）⑦searchUrl存词`java.put('key',key)`+v1明文接口回退+三通道bookList `$.book[*]||$..lists[*]||$.[*]`。方法主干 [references/方法-APP接口聚合书源拆解.md](references/方法-APP接口聚合书源拆解.md)，案例 examples/小小阅读书香之家app_s.wendulou.com.json + examples/小小阅读书香之家app_s.wendulou.com.md
...
> **HttpTTS 朗读引擎与音色切换面板（2026-09-12）**：起点 TTS 控制台（IFly+Minimax 双引擎）接成 6 个 Legado 在线朗读引擎沉淀的 **TTS 朗读引擎通用方法论** — ①**HttpTTS 全字段拆解**（id/name/url/contentType/pauseDuration/concurrentRate/loginUrl/loginUi/header/jsLib/enabledCookieJar/loginCheckJs）+ 源码级请求链路（HttpReadAloudService→AnalyzeUrl 绑定 speakText/speakSpeed/source）；②★**给朗读引擎加按钮的唯一官方通道**=编辑朗读引擎→⋮菜单→「登录」（要求 loginUrl 非空）→SourceLoginActivity(type=httpTts)→SourceLoginDialog 经典 RowUi 面板；按钮点击=`loginUrl+"\n"+action`，绑定 java(SourceLoginJsExtensions)/source/result/isLongClick，可用 java.clearTtsCache() 等 TTS 专属方法；③★**TTS body 是 JSON 时必须 JSON.stringify 拼**：写 `"text":"{{speakText}}"` 时正文里一个英文双引号就让 URL 选项 JSON 断裂→静默退化成 GET→**朗读无声且零报错**；正解 `<js>…JSON.stringify(bd)…</js>`（引号/反斜杠/换行全安全）；④★**语速语义推导**：speechRate=AppConfig.speechRatePlay+5（默认10），朗读界面显示值=speakSpeed/10（默认1.0，滑杆 max45）→ 站点 speed 0-100 默认50 要按「显示1.0=50」对齐（推荐 clamp(speakSpeed*5,0,100)，站点自带规则用 *2 偏慢）；⑤**音色切换实现**：source.put/get（BaseSource→CacheManager，键 v_{key}_{k}）存 prov/voice/smap + 规则内兜底（get 未设置返回 ""，保证导入即可用不点按钮）；button 的 viewName 不带引号包裹时**当 JS 表达式求值**→可做「当前音色」动态文字；FlexChildStyle 的 layout_flexBasisPercent/layout_wrapBefore 排版；⑥**loginCheckJs 把「没声音」变成看得见的报错**：拦 <800 字节响应里的 detail/<html/<!doctype 并 throw new Error，兼容 result.body 属性/方法两种 App；⑦**零双引号零反斜杠工程法**（fromCharCode(34)/(10)/(39)）+ MCP save_tts→get_tts 字节级回读校验→test_tts 字节数/耗时→同文本跨引擎比字节数+服务端 md5 证音色生效；⑧13 条踩坑清单（多段 Python 拼接漏 +Q+ 生成 `':'{` 语法错、% 格式化渗进 JS 变 NaN、new Function 不返回末表达式值必须 return eval(code)、错误响应被当音频、label 类型不渲染、jsLib 作用域跨版本不可靠）。方法主干 [references/方法-HttpTTS朗读引擎与音色切换面板.md](references/方法-HttpTTS朗读引擎与音色切换面板.md)，案例 examples/起点TTS朗读引擎_103.236.85.8.md + examples/起点TTS_朗读引擎6个.json（6 引擎，App 内 test_tts 实测 5 音色字节/md5 全不同）



> **登录UI V2 新版面板（2026-09-13）**：hanime1 v2.0 沉淀的 **登录UI V2（`loginUi={"version":2}`）通用方法论**（LegadoTeam 新版 SourceLoginV2Delegate/LoginUiV2 源码级拆解）— ①**V1 做不到的交互**：下拉 select(options 非 chars)、开关 toggle(value true/false)、按钮倒计时(countdown 防连点)、label 顶部消息板、error:{key:msg} 红字定位到具体输入框；②数据链路：loginUi(state) 返回 rows JSON，loginAction(action,state,form) 返回命令 {state/error/login/close}，state 动作间往返即面板内存；③硬约束：输入行必须 key、button 必须 action、key 与 action 各自 distinct（LoginUiV2.isValid 校验不过整面板渲染失败）；④Kotlin 默认参数 JS 须传全：evalLoginUiV2('{}',null,null)；⑤生产机制：hanime1 控制台 24 行（域名/hosts 下拉+测速选线+**代理开关**——header 用 <js> 动态注入 proxy 键解决视频 CDN 被墙，jsLib H1IPX 让 dnsIp 自动失效避开 proxy×dnsIp 互斥异常）+已登录访问 /login 返回 404 的站点陷阱识别（修复"无CSRF令牌"误报）+登录令牌 meta[name=csrf-token]/input[name=_token] 双通道；⑥**第一铁律：loginUi(state) 必须返回 {"rows":[...]} 对象包装，裸数组→parseRender null→对话框打不开**（v2.0 翻车实证，eval"能返回24行"≠能渲染）；⑦singleline 压缩器四续行陷阱；⑧md5 校验双坑（java.md5Encode 字符串重载才是标准 MD5、byte[] 重载非标准；JS length=UTF-16 code unit，emoji 长度对比不可信）。方法主干 [references/方法-登录UI-V2新版面板.md](references/方法-登录UI-V2新版面板.md)，案例 examples/hanime1视频_www.hanime1.me.json（check_source 1/1，真机 eval 全动作实测）。；**§7 控制台实战增补(hanime1 v3.4~v3.9)**：分支哨兵词冲突 / 多值不能当 select value(分组呈现) / 延迟后缀与解析解耦 / toggle 能力开关三件套(写变量+立即生效+关=可回退旧实现) / 账号密码三源兜底且用户清空优先 / 危险动作 countdown + robots.txt 轻量探测 / 消息板自定义分隔符换行


---

## 🧩 2026-09-14 难点方法论专题（13 篇批量沉淀 · 跨站可复用）

> 从 20+ 个生产书源的历史实战记录中提炼，按「被反复踩到的次数 × 复用价值」分为 T0（3 源以上反复再现）/ T1（单站但难度封顶）/ T2（工程方法论）。每篇统一结构：**现象谱系 → 源码级依据 → 解法阶梯 → 开箱模板 → 避坑清单 → 案例档案表**。

**T0 — 高频再现，不沉淀必继续翻车**

> **① AnalyzeByJSoup 状态污染与绕开总表**：**8+ 个源**反复再现的最高频难点（63sg/fdxrz/rrssk/zhaoshu/lzjxx/wn10/黑岩/hanime1）。三种表现脸：取空→"书籍总数:1"、字段串位→"作者显示成更新时间"、行分隔符被吞。源码实锤 `AnalyzeRule.kt:151` 对同一 content **复用 AnalyzeByJSoup 实例**（内部泄漏字段未定位，故解法是换通道而非改规则）；**新旧实例对照法**（新实例建基线 → 同实例按真实字段顺序连调复现）+ 反射 `getStringList$legado_app_appRelease`；四级解法阶梯（列表字段全 `@js:` 直取 / **XPath 通道免疫** / 结构规避 / 微型测试源）；`result` 在 5 个位置的类型矩阵（Element vs HTML 字符串 vs 净化后）；`absUrl` 恒空、`.replace` 需 `String()` 包裹、`tag.meta[...]` 恒失效等 10 条避坑 + 一分钟口诀。主干 [references/方法-AnalyzeByJSoup状态污染与绕开总表.md](references/方法-AnalyzeByJSoup状态污染与绕开总表.md)

> **② Cookie 标记型限频破解**：**6 站同款机制**（ppxsw/book630/xuken/ggd66/35sw/lianaiya）。服务端只检查请求**是否携带某 Cookie 键**、不校验值也不更新时间戳 ⇒ `search_time=1`/`ss_search_delay=1` 删掉即永久放行，干等与重试均无效（每次尝试还会重置倒计时）。三类限频鉴别**决策树 + A/B/C/D 四实验**；★`cookie.setCookie` 是**整体替换**语义（改单键必须读全文→过滤→回写全文，否则连登录会话一起丢）；本 App Cookie API 可用性清单（`java.setCookie` 无、`removeCookie(url,key)` 两参不存在）；IP 级惩罚窗口（xuken 可累计 1.5h+）与**离线 AnalyzeRule 验证不消耗配额**；`loginCheckJs` 把 0 结果变成"⏳限频"透传（IIFE + `result.body` 属性/方法双通道 + `throw new Error`）。主干 [references/方法-Cookie标记型限频破解.md](references/方法-Cookie标记型限频破解.md)

> **③ Legado 各版本行为差异对照**：跨版本分发的唯一依据，**22 项差异总表**（LegadoTeam / 官方 / legado-E·Lyc）。★头号差异：E 版 `getString/getStringList` **没有 `Mode.Js` 分支** ⇒ 列表/目录规则必须返回 **JSON 字符串数组 + 字段用纯键名 `$.name`**（LT 能跑的 `NativeObject` 数组在 E 版整源全空）；Kotlin 属性在 Rhino 里是**方法引用**（`gv()` 双兼容，35sw 静默失配案）；三作用域能力矩阵（jsLib 里 `java.ajax` 非函数 + `source` 未定义 ⇒ 参数注入法）；`Book.type` 位掩码 **video=4/text=8/audio=32/image=64** 与"刷目录 `removeAllBookType()` 冲掉详情页 type"（详情+目录双 `setType`）；-100 Cookie **配对错乱**三层真相与 v9g 全量发送对策；`AnalyzeUrl` 原生 `retry`/`dnsIp` 老版没有；**API 存在性判定方法论**（含 ppxsw 把 `getVerificationCode` 误判为"不存在"的复盘：`typeof` 只能证有不能证无）+ 如何做两版源码 diff。主干 [references/方法-legado各版本行为差异对照.md](references/方法-legado各版本行为差异对照.md)

> **④ URL 模板 `{{}}` 求值语义与 DNS 选线**：★关键认知=`{{}}` 在**每次请求**时二次求值 ⇒ `bookUrl`/`chapterUrl`/`exploreUrl` 全部可动态自愈（域名轮换书架永生）。模板七坑（域名段空→整段被吞、单模板产出被小写化、`$` 未定义、`searchUrl` 用 `@js:` **静默**退化成相对路径、裸双引号毁 URL 选项→静默变 GET、**多个 JSON 对象→NPE**、规则里禁真实换行）；URL 选项全清单（`dnsIp/resolveIp`、`retry:3` 原生但 **POST 撞 oneShot**、`timeout`、`charset`）；`buildScopedDns` 语义（只劫持本域、多 IP OkHttp failover、去 Cronet 保留 SNI/证书校验、**与 proxy 互斥**、写进 header 无效）；hosts 池→判活→测速→落库→消费→手动自救六步闭环；**探针法**（微书源 + `.invalid` 废域名 + `get_http_logs` 看最终请求 URL，一次定位模板问题）；变量双通道与 ★**Rhino JsNull 是 truthy**（`source.get` 未设置键 → `if(dv)` 为真、`||` 兜底失效，必须 `String()` 三态归一化）。主干 [references/方法-URL模板与DNS选线.md](references/方法-URL模板与DNS选线.md)

**T1 — 单站点但难度封顶**

> **⑤ 字体反爬三代方案演进**：三型判别（A 图标字体 PUA / B 会话置换全文混淆 / C 其实是加密）。B 类破解主线：**固定 `reader_guard_id` Cookie 锁定置换表** → ★离线**字形位图匹配建表**全算法（包围盒裁剪 + 统一 36×36 + 高斯模糊 + L2 归一化 → 一次矩阵乘算全表余弦相似度 + **双参考字体融合**（黑体 wqy-microhei+NotoSansSC，宋体要换 Serif，选错族形近字大面积翻错）+ **margin<0.25 入 weak 待复核清单** + 已知样本验收）→ 位置对齐双串 K/V 外置托管（dpaste 365 天）→ **生产级运行时加载代码**：内联在 `@js:`（jsLib 不能发请求）+ `typeof result` 双兼容 + **哨兵字符对校验**（`戗→气`/`褛→斗`，比长度判据强，能识破 CDN 错误页）+ 未知字符原样保留（绝不 `s+=o[c]`）+ `source.setVariable` 缓存。★变量 API 澄清：规则侧 `getVariable()` **无参、书籍级**（存库），`loginUrl`/`eval_js` 侧只有 `put/get`（CacheManager，重启失效）。含 13 步流程清单与 13 条避坑。主干 [references/方法-字体反爬三代方案演进.md](references/方法-字体反爬三代方案演进.md)

> **⑥ 多层密钥 / AES 信封 / 轮换签名逆向手册**：密文**分型表**先行（base64 / LZString / 16 倍数 AES / 8 倍数+响应头 DES / 全汉字读不通=字体）。密钥定位四法（JS 特征枚举、**文件名随机化 → `?_=`+`Date.now()` 强制 CDN 回源**、响应头 `accesskey/keystring` 旁路、同响应信封自解）。★体系 A（wendulou 动态种子信封）：`base64Decode`=16B key 种子+密文+16B IV 种子，`key=SHA256(种子hex)`，`IV=md5(iv种子).hexdigest()` 取 **32 hex 的前 16 个 ASCII 字符**逐位 ⊕种子⊕0xFF。★**CBC 定位铁律：密钥错→全乱；IV 错→只有首块 16 字节乱**（Python 用 raw digest 当 IV 就是这一症状）；hutool 宽容 base64 缺 `=` 而 Python 严格。体系 B（rrssk `class AESCrypt` + `iv=6051140156343299`，请求体也要 AES）+ 新旧格式兼容提取；体系 C 晋江双层 DES。★**花括号配平法**从每日轮换的混淆 JS 里切函数再 `eval`（站点天天重混淆但 `{}` 结构不变 → 同一份规则跨 30+ 天不用改）。base64 **随机对象名**用 `_keyStr` 特征匹配 + 组合规则拿不到 `<script>` 必须纯 `@js:`。★**破坏性单变量实验**（wendulou `sign=deadbeef` 照常返回 ⇒ 真鉴权在 UA 尾部 `_{package}`，省 90% 工作量）；"实验者也是变量"纪律。主干 [references/方法-多层密钥AES与轮换签名逆向.md](references/方法-多层密钥AES与轮换签名逆向.md)

> **⑦ 图片不显示排查总表（加密之外的三条路）**：四象限归因（先问"URL 提取到了吗"——`?verify=` 后缀会让"扩展名后紧跟引号"的旧正则全失配）。★**路一 verify 签名不绑主机** ⇒ 同 token 打 img1~img5 全 200 ⇒ 可自由换 CDN 主机；签名绑定维度判定表（路径/host/Referer/UA/时间桶），时间桶内 URL 稳定才能命中缓存（别加 `Date.now()`）。★**路二 SNI 封锁换主机名而非换 IP**：查 CNAME 链用 **CDN 侧原生主机名**（`vdownload.hembed.com` → `1497203185.rsc.cdn77.org`）。★**路三 `blockNetworkImage` 源码真相**：`BackstageWebView.createWebView()` 硬写 `true`、`WebViewPool` 只在 `release()→onPageFinished` 复位 ⇒ 池化 WebView 竞态会让 useweb 里**所有网络图画不出而 `data:` 正常** → 规则侧下载 + `Base64.encodeToString(b,2)` 内嵌（五护栏：协议/≤230KB/contentType/长度/全 try）+ 候选链 `data→pu→https→http→换host` + `onerror` 逐试与状态回显；连带坑=**data 图塞进有长度闸门的变量会被静默丢弃**（9000→400000）。连通性判活：根路径任意响应码=通，但 **cdn77 GET 200 / HEAD 恒 403**（方法被拒≠被墙）；`java.ajax` 有时返回异常文本而非抛。★**沙盒会完全误诊**（沙盒 img5 全 200、用户手机被 RST）⇒ 连通性只能在手机侧 `eval_js` 实测。另有 `imgPattern` 硬性双引号要求、HTML 属性无引号禁空格、`java.webView` 渲染探针（js 必须写成 IIFE 表达式）。主干 [references/方法-图片不显示排查总表-加密之外的三条路.md](references/方法-图片不显示排查总表-加密之外的三条路.md)（前篇 [方法-图床AES加密图片解密](references/方法-图床AES加密图片解密.md) 不重复）

> **⑧ WAF / Cloudflare 间歇故障与"站点到底死没死"的判活纪律**：★第一原则=**判"站点失效"前必须做七维复测矩阵**（多时点/多通道/多协议/多端口/多 UA/多路径/查镜像），并附 **ggd8 误判复盘**（昨日全端口 SSL handshake aborted → 次日复测发现已迁 CF、无挑战无频控，白扔一天）。WAF 家族分型对策表（lianaiya 型 `503+var token` 自研挑战 / **GoEdge**（307 `/WAF/VERIFY/CAPTCHA`、**移动 UA 豁免 GET 但 POST 强制人机** → `getVerificationCode` 弹窗、回退请求三处全带移动 UA）/ CF 挑战型 / **CF 抖动型**（间歇 RST）/ **error code 1034** / 裸 UA 403 / 限频 / 需登录）。★**UA 的三种作用**：决定放行、决定 PC/移动**两套模板**（规则会全线失效）、**17mb 系完整 Chrome UA 反被 RST**（短移动 UA 放行）⇒ 四种 UA 枚举实测。间歇 RST 工程化四件套（全链路 `retry:3` + **异常文本校验** `/javax\.net\.ssl|Exception|reset by peer/` + 梯度等待与降级路径 + TLS 冷连接先打首页）；★自伤型故障（debug 连打把 IP 打进长惩罚 / 触发 CF 挑战）与离线验证纪律；判活决策树 + 源备注"连通性档案"模板；**镜像"能打开"≠"能换"**（换域名=全书架失效，须先验书籍 ID 是否互通）。主干 [references/方法-WAF与CF间歇故障及站点判活.md](references/方法-WAF与CF间歇故障及站点判活.md)（前篇 [WAF挑战过盾指南](references/WAF挑战过盾指南.md) 互补）

> **⑨ 付费墙 / 假登录墙 / 死链的甄别与用户提示设计**：★**"~~~~~请登录后在继续阅读本部小说！~~~~~" 多半是付费墙伪装**（63sg/35sw 实测：注册+登录+加书架后第 100 章仍锁，www 与 m 双站同锁）。**五步甄别流程**（存证锁章→注册→登录并**复核会话**→带 Cookie 复测同章→仍锁即付费墙，立即停止投入）；★登录态判据优先级：**仅登录态才渲染的元素（退出链接）> 登录入口链接 > 页面文案哨兵**（zhaoshu 把登录框副标题当未登录哨兵 ⇒ 登录永远失败的事故复盘）；302 会吞 flash 与改写响应体 ⇒ 必须再 GET 复核。三级付费粒度（书级前缀免费 / 章级手打中与转码失败 / 整本版权缺失）各自处置；**SEO 假正文陷阱**（占位页 `#ctxt` 里塞"书名+简介+展开全部"）与四条防御（内容专属哨兵、与 intro 重合度双判、优先找 ajax 接口旁路、别把"有字"当"有正文"）；★提示文案四原则（说清"是什么+为什么+用户能做什么"、开头留换行、不放网址、**列表字段必须纯文本**）——"给一句人能读懂的话，永远优于 `ContentEmptyException`"；章级兜底"坏一章不崩整本" + CDN 403 降级保底 120 章；**不要试图绕付费墙**（技术无效+风险，Legado 的换源才是正解）；9 站对照档案表（同为"请登录"，63sg 假墙 / ppxsw·book630 **真登录墙**值得做登录）。主干 [references/方法-付费墙与假登录墙甄别.md](references/方法-付费墙与假登录墙甄别.md)

**T2 — 工程方法论（不是站点知识，但最救命）**

> **⑩ 书源交付质量闭环**：★信条=**"MCP 回显已保存 ≠ 内容正确"、"功能跑通 ≠ 字节一致"**。三次事故开场（`save_source` **整体替换**语义误传只含 comment 的 JSON ⇒ App 内全部规则被清空；手抄/传输层双重转义丢一个 `;`/一个字母；68~89KB 大源截断）。六步流水线：①构建脚本是唯一事实源（**一律以上一版 JSON 为基线**、整块按行替换、唯一字面锚点+顺序断言、产出缩进版+单行版）；②语法体检（抽全部 JS 块 `node --check`，**`loginUi` 除外**——宽容解析器必误报）+ `new Function(...,'return eval(code)')` 语义仿真；③**零双引号零反斜杠纪律**（`fromCharCode(34/92/10)`、`[0-9]`/`[.]` 代正则、`loginUi` 用 `<js>JSON.stringify([...])</js>` 运行时产引号、唯一例外 `header`）；④落库三语义（同 URL `REPLACE` 不产生重复源、重置 `respondTime/lastUpdateTime` 故基线须排除、`enabledExplore` 可能被重置要复查）；⑤★**N 字段 md5 回读闭环**（本地 len+md5 基线 → `eval_js` 绑定源逐字段回算 → 差异字段 **150 字符分段 md5 二分** → charCode 逐字打印；度量三坑：`String.getBytes('UTF-8')` 不存在须 `Charset.forName`、Rhino length 是 UTF-16 而 Python 是码点（emoji 差 74~155）**故只比 UTF-8 md5**、`workspace_read_file` 显示层也吞字符）；⑥功能验收 debug+check_source。大源通道：dpaste `api/v2/` **带尾斜杠** + 手机侧**只拉一次**（二次拉 429 HTML → 被当 JS 源报"XML 语法错误"）+ `java.openUrl('legado://import/bookSource?src=…')`。构建脚本自身四类 bug（`.*?` **跨块吞锚**把代码插进别的 if 块而 `node --check` 照样通过、漏 `+Q+`、`%` 渗 JS、heredoc 吃引号）+ **30 项交付 checklist** + 最小 md5 脚本。主干 [references/方法-书源交付质量闭环.md](references/方法-书源交付质量闭环.md)

> **⑪ 静默失败定位与探针法**：★贯穿原则="**不要推理，要打印**"——Legado 大部分规则写错不报错只返回空，故"获取成功 / 总数 1 / 目录获取失败"都不可信。**15 类静默失效总表**（IIFE 漏 return、`searchUrl@js:` 退化、`kind` 唯一走 `getStringList` 强转、`result.body` 属性 vs 方法、JsNull truthy、`tocUrl` 放错位置、顶层 `checkKeyWord` 被丢、组合规则拿不到 script、`chapterUrl` 的"选择器@js:"不执行、jsLib 网络静默空、禁用写法、`tag.meta` 恒空、`absUrl` 恒空、双 JSON NPE、规则含真换行）。★**IIFE 漏 return 专题**：lwenxsw 同一源三字段同时犯（正文 `ContentEmptyException`、发现"发现规则为空"被写进 group、搜索**完全静默 0 结果**），根因是把"脚本顶层末表达式值"误当成函数内的返回语义；构建脚本要断言每个 `@js:(function(){` 块含 `return`。四种探针：①微书源 + `.invalid` 废域名看 `get_http_logs` 最终 URL；②★**`eval_js` 真机离线复现**（`source.getContentRule()` 取真实规则 + 伪造 `result/baseUrl` 跑、或 `new AnalyzeRule().setContent` 复现污染；四个必知失真：**改规则必须先 save 再测**、函数作用域 eval 的 function 不外泄、重声明同名函数不覆盖 SharedJsScope 旧 jsLib 绑定、`getTocRule()` 而非 `getRuleToc()`）；③最小对照源 `__diag_*`（改一个字段同跑，验完删）；④破坏性单变量矩阵（含副作用警告）。附 Rhino/jsoup 语法坑表、反射探测（`getClass()` 返 null、`Class.forName` 被封、`$legado_app_appRelease` 后缀、R8 改名不可见清单）、症状→探针定位流程、三句口诀。主干 [references/方法-静默失败定位与探针法.md](references/方法-静默失败定位与探针法.md)

> **⑫ 目录顺序与章节去重铁律**：两条源码级铁律（★`BookChapter.equals` **只比 url** + `LinkedHashSet` ⇒ 同 URL 章节被合并，共用一页必须加 `#fragment`；★阅读**按出现顺序接收、不替你排序**，治理全在 `chapterList` 内）。官方抓目录两模式：`nextTocUrl` 单 URL 走 **while 链式（天然保序）**、多 URL 走 `flow.mapAsync` 并发（按发射顺序 await 但页序不保证）⇒ ★**"并发抓全页"能否用的判定条件 = 详情页是否内联了章节**（63sg 独立目录页并发 `option@value` 拿下 1699 章 ✅ / xuken 详情页含前 100 章则并发错序 ❌ → 改链式每页只返回 selected 的下一个 option）。六类脏数据与对策：①"最新章节"倒序区（6 站同形，取第二个容器/`#list-chapterAll`/`a[name=all]~ul.p2`）②隐藏副本+页尾垃圾+其他书推荐（★找**黄金特征 class** `br-b-1`，比索引稳十倍）③幽灵页与 **ymxwx 镜像陷阱页**（ml36 实为第 1 页 → select 分页上限校验阻断）④死章（`…/0.html`）⑤占位项空 href（白名单 `^[0-9]+[.]html$`）⑥CDN 403 降级链**保底 120 章优于空目录**。排序键优先级（标题"第N章" > 章节号 > aID > 不排；**aID≠章节序**；取不到序号保持稳定排序不堆一端）。虚拟章节 `data-rid='open-'+encodeURIComponent(url)` + `baseUrl#reply-` 双保险，⚠️ 必须 `split('#')[0]` 清洗上游意外 fragment。`tocUrl` 位置/无 `||` 兜底/`chapterUrl` 的 JS 形态差异；串章三防护（只跟同章分页形态 URL、`lastread.set` 同章校验、**空串是安全终止信号**）+ 硬切分页 `&nbsp;` 碎片正则；**"顺序 0 违规"验收法**（单调性检查 + 五点抽样跑正文 + debug 页间 ≥1.5s）；12 站档案表。主干 [references/方法-目录顺序与章节去重铁律.md](references/方法-目录顺序与章节去重铁律.md)

> **⑬ 详情页交互按钮选型树**：官方三条简介通道源码级（`BookInfo.kt:125` 以 `<usehtml>`/`<useweb>`/`<md>` 开头则**原样存、跳过净化**）。★**最重要事实：`<button>@onclick:JS` 走 `BookInfoViewModel.onButtonClick → source.evalJS(click)`，跑的是完整 Rhino JsExtensions 环境，不看 `@JavascriptInterface` 注解** ⇒ 在 `<useweb>` 页面里调不到的 `java.startBrowser`，在 `<usehtml>` 按钮里能真弹内置浏览器。五通道能力边界总表 + **选型决策树**（点一下执行 JS → A `usehtml` ★默认；需要活的界面/随事件自更新 → B `useweb`+`setInterval`；打开另一本书 → E 深链嵌进 A/B；每章正文都要按钮 → C 正文 `<usehtml>`（`BookContent.kt:243`+`TextChapterLayout:689`）或虚拟章节；功能入口列表 → D 订阅源）；A/B **互斥**，生产要**双实现 + 登录 UI 开关留降级路径**（hanime1 `H1OLD()` 快照）；useweb 三条由源码推出的纪律（`activity=null` ⇒ `java.request("run")` Promise **永不 resolve**；`substring(8, lastIndexOf("<"))` 截断 ⇒ 用户文本全转义、闭合标签必须最后；`java.ajax` **冻结 WebView JS 线程** ⇒ 高频动作数据预置只读内存）；`refreshBookInfo()` 在播放页是空操作 ⇒ 只能 useweb 活视图 + `callBackJs/eventListener=true`（切集必跑、不受正文缓存影响、**两处双写**）；★20 项**桥能力矩阵**（"注解只管 WebView 桥，不管 Rhino eval"）；实时 ajax 重取签名直链防 1 小时过期；`legado://import/addToBookshelf?src=` 三级书源匹配链路 + `java.openUrl` 比 `window.open(_blank)` 稳；六条跨通道硬约束（onclick 禁裸 `<` 与 `&&`、列表 intro 必须纯文本、useweb 内禁 http 可点链接、订阅源 sourceUrl 撞书源 URL 会被覆盖、sourceIcon 别用 SVG、零双引号）；验证法（eval_js 完整复现 onButtonClick 路径 + `java.webView` 探面板 DOM + 桥缺失显式降级）+ 三档实现模板。主干 [references/方法-详情页交互按钮选型树.md](references/方法-详情页交互按钮选型树.md)（详见 [方法-简介useweb内嵌页面与浏览器调起](references/方法-简介useweb内嵌页面与浏览器调起.md)、[方法-登录UI-V2新版面板](references/方法-登录UI-V2新版面板.md)）
...
> **详情页写操作按钮与关系态显示（2026-09-16）**：主干 [references/方法-详情页写操作按钮与关系态显示.md](references/方法-详情页写操作按钮与关系态显示.md)（30.9KB）。专治三类被误当成"刷新坏了/按钮坏了"的问题：①**下拉刷新简介面板像没反应** —— 刷新链路是好的（`BookInfoActivity:1173 refreshLayout→refreshBook→BookInfoViewModel.loadBookInfo`，详情零缓存），病根在**面板数据源**：TTL 缓存的触发哨兵全源码无人写入（"下拉刷新即可载入你的列表"＝空头支票）；给出**哨兵空头支票检查法**（grep 键名确认有 setter）与 **TTL 面板三件套**（渲染侧自愈补拉 + 冷却戳写在发请求**之前**、失败也冷却 + 显式「🔄」清缓存强拉重绘），收尾型扫描按钮必须回写缓存+`java.refreshBookInfo()` 消灭"再下拉一次就能看到"。②**关注/收藏类写操作** —— ★**从站点自己的 main.js bundle grep API 定义表**一次拿到「路径＋参数是 UUID 还是用户名＋动词＋幂等返回体＋i18n 官方文案」，不必撞路径；★成功判据要用**返回体行 id**而非"没报错"（iwara 幂等关注重复返回同一行 `{"id":59252845}`）。③**√ 已关注怎么显示** —— ★关系态是零成本的：`loginHeader` 由 `getHeaderMap(true)` 自动附加同域请求，于是普通详情请求就是登录态，业务响应内嵌的 `user.following`/`liked` 即真值（真机：关注前 false→关注后 true，`followedBy` 恒 false＝"TA 关注我"）；给出**四层阶梯**（L1 内嵌字段→L2 专用接口→L3 本机账本 10min 只填空窗→L4 显式核对）与**三态 UI**（未关注/√已关注/是自己的/未登录整行不渲染）＋显示名转义与 onclick 参数两套清洗。★**本 App 发不出 DELETE**：`AnalyzeUrl.kt:268` 白名单只认 POST/HEAD、else→GET，`RequestMethod` 枚举仅 `{GET,POST,HEAD}` ⇒ `method:"DELETE"` **静默变 GET**；三条 POST 覆盖绕路（`?_method=DELETE`/`X-HTTP-Method-Override`/body `_method`）与原始 TLS 套接字手写 DELETE（`SSLSocketFactory`+`createSocket(plain,host,443,true)` 保 SNI、逐字节读绕开被 JavaClass-shutter 封的反射数组、手工解 chunked）**实测全废**（CF Read timed out），结论＝**取消类动作必须降级**：`longToast` 讲明限制 + `java.startBrowser(对象主页)` 把用户送到能操作的地方。另含 18 条避坑（列表按钮上限要对齐真实账号数据量：真机 14 个列表 vs 旧上限 12）与**验证纪律：新断言必须先负对照**（同一套断言跑改动前的旧 JSON 必须挂，本轮当场抓出自己写宽的一条正则）。案例 `iwara·MMD视频 v6.3→v6.4→v6.5`，成品 jsLib md5 `c55e60ffb7c69e53c52b5d114306eab0`。

> **深链保存书源与 dpaste 通道（2026-09-16）**：新增 `references/方法-深链保存书源与dpaste通道.md`（27KB / 11 章 / K1~K28 避坑）+ `scripts/dpaste_upload.py`（一键上传 + 字节校验 + **打印唤起代码**）。★核心信条=**「上传只是半程，唤起才算交付」**——只上传 dpaste 不唤起，App 毫无反应，任务等于没做。①**深链全谱**：`legado://import/{path}?src=`（`yuedu://` 等价别名），path 全表（`/bookSource` **万能入口**吃 JSON/JS源/URL/内联文本；`/rssSource`；`/httpTTS`；`/replaceRule`；`/addToBookshelf`；`/auto` 只嗅探 JSON/zip 不吃 JS 源），AndroidManifest `exported=true`+`BROWSABLE`，源码确认 **5 处触发方**，★`java.openUrl` 对 `legado://` 特判**直接 startActivity、不弹外部浏览器确认框**（普通 http 才走 `OpenUrlConfirmActivity`），故书源里做「一键导入另一个源」比 `window.open` 更稳。②★**唤起导入三步走**：造文件 → 上传中转 → `java.openUrl('legado://import/bookSource?src=' + java.encodeURI(raw))`；**成功硬证据 = HTTP 日志出现 `GET <raw> -> 200`**（本轮 #2351/#2353/#2355 三次实锤）。③★**三类资源格式差异（最易翻车）**：**订阅源的 URL 分支强制 `fromJsonArray` ⇒ 单对象必报 `ImportError: java.lang.IllegalStateException: Expected BEGIN_ARRAY but was BEGIN_OBJECT`，永远写数组 `[{...}]`**；书源/TTS 单对象亦可（本轮**正/反向双验证**：先跑预期失败的用例再跑修正版）。④**dpaste API 契约实测**：`api/v2/` **必带尾斜杠**（漏了返回 API 文档 HTML 且状态 200）、`expiry_days` 范围 **1~365**（不传 400）、★**拼成 `expiration_days` 会 201 但静默只有 7 天**、raw 是 **`.txt` 不是 `.raw`**、匿名粘贴**公开且无法删除**（别放账号/Cookie/token）、≤1 req/s；★**429 的完整误诊机制链**（返回 HTML → 非 JSON → `JsSourceConfig.extract` → Rhino `Parser` → `InternalError: XML 语法格式错误`）——看到「XML 语法错误」先查 src 是否返回了 HTML。⑤三条投递通道对照（深链 / **网络导入**=内联粘贴零依赖排障首选 / 扫码=QR 只放 raw URL）与 `src` 六种形态（单对象、数组、`{"sourceUrls":[...]}`、绝对 URL、file URI、JS 源文本，含 `#requestWithoutUA`）。⑥K25~K28：★**`java.md5Encode(byte[])` 不存在**且传 byte[] **静默得到错误 md5**（只传 String）、Rhino `length` 是 **UTF-16**（emoji 差 1，只比 UTF-8 md5 与字节数）。案例：545B 书源含 emoji/反斜杠/双引号/换行/制表，**端到端 md5 完全一致**；订阅源反向验证报错原文、TTS 单对象验证均已留档。
...
> **legado-E 兼容改造实战（2026-09-21）**：[方法-legadoE兼容改造实战](references/方法-legadoE兼容改造实战.md) —— ★**书源在 legado-E 上"登录界面空白/列表字段全空/目录失败/按钮没反应"的成套修法**。E 版源码级契约 14 条（`loginUi()` 只认 V1 数组且 `RowUi` 仅 4 字段 → `{"version":2}` 必空白；按钮返回值被 `runCatching` 吞 → 必须自己 longToast；✓ 按钮不注入 result → `getLoginInfoMap()` 兜底；`getElements` 是 `as List<Any>`；`BookContent` 无 callBackJs/eventListener/isVideo；`BookInfoActivity` 无 useweb；`BookSourceType` 无 video；`UrlOption` 无 dnsIp/timeout；`CookieStore` 无两参 removeCookie；缺 copyText/openVideoPlayer/refreshBookInfo，get/post/head 仅 2 参）；**改造五步法**（判定→建 `##兼容版` 独立源→loginUi 降级 V1→loginUrl 追加 `H1V1(act,Jv,Sv,m)` 薄分发 + `login()` 双通道→列表内预生成字段+纯键名）；**V1 面板模板与 6 条硬约束**（label/select/toggle 静默不渲染；分区标题用 inline JS；name=回填键；无 value 字段→靠 `h1uiSave` 写 `putLoginInfo`）；**三层验证法**（静态契约→LT 真机实测→E 版源码核对+等价模拟：同条目取 `title` vs `@js:String(result.artist)` 后者为 undefined 即证差异）；K1~K20 避坑。案例：[hanime1 E兼容版](examples/hanime1_E兼容版_www.hanime1.me.md)

> **legado-E 兼容改造实战（2026-09-21）**：[方法-legadoE兼容改造实战](references/方法-legadoE兼容改造实战.md) —— ★**书源在 legado-E 上"登录界面空白/列表字段全空/目录失败/按钮没反应"的成套修法**。**开头即铁律：拉 E 版源码必须先确认默认分支（Luoyacheng/legado-E 是 `main` 不是 `master`，否则会误判 8 项能力）**；E 版 main 能力表（✓ useweb/usehtml+onButtonClick、✓ video=4、✓ dnsIp、✓ openVideoPlayer、✓ select/toggle、✓ 3 参 get；✗ 不认 `{"version":2}`、✗ callBackJs/eventListener、✗ 无两参 removeCookie、✗ 无 copyText/refreshBookInfo）；**真正必须改的只有两件**：① loginUi 数组化（+ `H1V1(act,Jv,Sv,m)` 薄分发、longToast 自报结果、按钮代替下拉、查看当前配置回显）② 列表字段纯键名（计算搬进列表 JS）；防御性兼容 `rmck`/get 双通道/缺失 API try/`h1uiSave` 回填；**三层验证法**（静态契约→LT 实测→E 源码核对+等价模拟）；K0~K20 避坑。案例：[hanime1 E兼容版](examples/hanime1_E兼容版_www.hanime1.me.md)


> **聚合源状态漂移修复与选站面板（2026-09-23）**：[方法-聚合源状态漂移修复与选站面板-rrssk](references/方法-聚合源状态漂移修复与选站面板-rrssk.md) —— ★**靠 sourceVariable 全局 server 分发的多域名聚合源必得"漂移病"**（书架书正文为空=server 停在 boluomao 死站后家族书走 data-obf 分支取空 / 搜索中转失败 / check_source 洗变量炸 `/list-` 相对 URL）；**根治=URL 自治五层**（`SRV(book.bookUrl/baseUrl)` 推导域名同域优先全局 scheme、chapterList 输出绝对 URL+`chapterUrl=$.chapterurl`、`dataEncrypt` 显式缓存键+scheme 兜底、**别名书按页面 data-aid 实际值自纠**（hggjfg→hgg）、toc 取值链 `book.url→tocUrl(http)→baseUrl(http)`）+ `{{NBK(book)}}` 空书名保护；**选站三件套**：登录面板（`loginUi=<js>` 运行时从 hosts 生成 47 行网格 / action 跑在 IO 线程可过 refreshExplore 主线程检查 / `BaseSource.evalJS` 单参 Rhino 调不到 / login() 仅非空文本行才调）+ 发现下拉（**SET 移进 select action**——evalButtonClick 带 infoMap 绑定，kinds 求值不回写防过期 live InfoMap 覆盖）+ 下拉下方**状态栏**（Type.button viewName 动态读 GET 真值，3~19 单引号字面量规则）；**check_source 校验器三坑**（无 UI 态 infoMap 洗变量 / 哨兵只认空串与 {}→改 `!GET(key)` / kinds 两级缓存）；**模板引擎与 htmlunit 四铁律**（`{{}}` 无括号=属性路径恒空、带括号=JS 求值 / Kotlin 空 Java 串 truthy 须 `String(x).length>0` / java.lang.String 无 .length / evalJS 默认参数 Rhino 单参调不到）；**K1~K18 避坑 + Node 行生成仿真 / 真机 kinds 求值 / 污染态对照 / check_source×3 / 全字段 md5 / dpaste 深链交付法**。架构篇：[聚合源多域名架构设计](references/聚合源多域名架构设计.md)；案例：[rrssk聚合_44域名](examples/rrssk聚合_44域名多站聚合.md)
...
> **方法-小说漫画视频三合一书源详解（2026-09-24）**：禁漫天堂Pro v1~v14 蓝本的"一个书源三种形态"总纲——四招架构（模式变量/字段整体分发/统一解析器/按URL分支）+ jmode发现页下拉 + #!搜索前缀 + 作者字段宁缺毋滥特征行 + kind芯片头部=作者原生搜索 + book.type动态设置 + 收藏闭环(重复收藏=已收藏/缓存优先) + 标签数据化扩充 + K1~K14 + 移植清单。案例 examples/禁漫天堂Pro_三合一.{json,md}
> **方法-AEGIS-ALTCHA验证与WebView通道书源（2026-09-25）**：69shuba.tw 蓝本的 AEGIS+CF 双防线首例——协议逆向（定制PBKDF2变体 SHA256^cost(salt‖nonce‖counter4BE) / aegis_altcha_verify POST / ja4+ip 写入 challenge data）；★判活铁律（keyPrefix=32hex(16字节)=发给非浏览器指纹的deny挑战、数学不可解、参数纯诱饵——别硬写PoW求解器）；★WebView通道传输层（java.webView(null,url,null) 走真Chromium栈被视作浏览器 → loginCheckJs 拦截"altcha-widget" → new StrResponse(url,src) 整体替换响应 → 上层规则照常写）；△Cookie跨栈不升级实锤（WebView凭据回灌OkHttp仍403=按请求指纹逐次判级）；正文规则内置二次自愈兜底；登录面板检查访问状态/网页过盾/打开网站三按钮+三档Toast提示词；性能模型（每页1次无谓403+1次WebView≈2s/章）+K1~K8+移植清单。\n> **同站双通道整合·delayed-webview 主动预取式（2026-09-25）**：`https://69shuba.tw#delayed-webview` 变体不再挨403——searchUrl 直接带 URL 选项 `{"webView":true,"webJs":"document.documentElement.outerHTML","webViewDelayTime":5000}`，详情/目录/正文/发现用规则内 `java.webView('',url,js)` 或 `java.webViewGetSource('',url,js,'',false,4200)` 直走内核；★delay 语义=onPageFinished 后再多等 N 毫秒才执行 webJs，给页面内 ALTCHA 自动 PoW 留完成时间（本站搜索5000/发现4200/正文0；无js无delay时系统兜底900ms；delay 是买稳定性不是买速度）；★全签名源码核准（java.webView 3/4参 + webViewGetSource 6参(sourceRegex,cacheFirst,delayTime) + GET走loadUrl真栈/POST走loadDataWithBaseURL(res.body) + UI线程禁调 + blockNetworkImage=true + webJs 优先于 jsStr + null结果按[200..1000]ms重试30次/60s超时 + onReceivedSslError恒proceed）；★四件套整合模板=主动打底+loginCheckJs兜底+正文自愈+面板过盾按钮；★双通道六环节对照表（请求次数：A=403+WebView2次 vs B=WebView1次 / 兜底：A全链路自动 vs B逐入口裸奔 / 上层规则：B的webJs里querySelectorAll?.*可选链全可用不受Rhino语法限制）；适用边界（ALTCHA自动过盾型✅/需人工点击的Turnstile打勾❌）；K1~K13+双版本两铁律。案例 examples/69书吧_www.69shuba.tw.{json,md}

> **💰 次元姬小说 api.hwnovel.com（2026-09-25）**：App官方API书源全案——
> ①**设备号风控**：服务端设备库（已知号200 / 新16hex→`400网络繁忙` / 有序号403 / 缺头500）；同IP好号新号对照排除限频；无离线激活接口→共享已验证设备号+登录面板「设备号」自定义`source.put('cydn')`+header写`@js`动态生成+loginCheckJs拦截。
> ②**四件套加密门禁**：DES-ECB/PKCS5(key=ZUreQN0E)加密参数+requestId=UUID无杠+sign=MD5(base64拼串).大写+★timestamp必须JSON数字(字符串→500)；GET query/POST body同构；必带4客户端头version3.4.8/platform1/channel100/deviceno；★签名无时间窗无重放→bookUrl直接嵌永久签名URL书架长生。
> ③**crates.io拆包逆向**（GitHub被墙降级路径）：/crates/{name}/{ver}/download 源码tar.gz+版本考古（0.20.x已移除模块、0.19.2最高版）。
> ④**VIP未购code=101不是错误**：~200字预览密文仍可解密；payAction复用chapter.url里param密文拿bookId/chapterId再POST order/consume。
